...
startingPos = [18,1,6]
path = [[18,1,6,0]]
def pathFinder(time, down, across, pointer):
    if time + 1 == len(map):
        print("no more time")
        return
    else:
        ###map[t][x][y]
        if pointer == 0:
            if len(map[time + 1]) > down + 1:
                if map[time + 1][down + 1][across] == 0:
                    path[-1][3] += 1
                    newCoords = [time + 1, down + 1, across, pointer]
                    if newCoords[1] == 6 and newCoords[2] == 0:
                        #return if exit reached
                        return newCoords
                else:
                    newCoords = [time, down, across, pointer + 1]
            else:
                newCoords = [time, down, across, pointer + 1]
        elif pointer == 1:
            if map[time + 1][down][across + 1] == 0:
                path[-1][3] += 1
                newCoords = [time + 1, down, across + 1, pointer]
                if newCoords[1] == 6 and newCoords[2] == 0:
                    #return if exit reached
                    return newCoords
            else:
            ...
            if map[time + 1][down][across] != 0:
                if len(path) <= 1:
                    print("there is no path that does not lead to death")
                    #return if exit reached
                    return [0,0,0,0]
                else:
                    path.pop()
                ...
                newCoords = [time + 1, down, across, pointer]
        path.append(newCoords)
        pathFinder(*newCoords)
    return [0,0,0,0]
print(pathFinder(*startingPos, 0)[0])
